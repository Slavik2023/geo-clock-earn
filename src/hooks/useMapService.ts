...
export function useMapService({ onMapLoad }: UseMapServiceProps = {}) {
  ...
  const loadMapService = () => {
    // OpenStreetMap doesn't need API loading, just set loaded to true
  ...
  return {
    ...
    handleGetCurrentLocation,
    setSelectedLocation,
    mapServiceLoaded: serviceLoaded
  };
}
